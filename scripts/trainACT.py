...
def main(args):
    ...
    # fixed parameters
    state_dim = 7 # action dimension in action sequence
    input_to_model_low_dim = 9
    lr_backbone = 1e-5
    backbone = 'resnet18'
    if policy_class == 'ACT':
        enc_layers = 4
    ...
    config = {
        ...
        'state_dim': state_dim,
        'input_to_model_low_dim': input_to_model_low_dim, 
        'lr': args['lr'],
...
def eval_bc(config, ckpt_name, save_episode=True):
    ...
    input_to_model_low_dim = config['input_to_model_low_dim']
    ...
    video_writer = None
    if write_video:
        video_writer = imageio.get_writer(ckpt_dir+'/'+ckpt_name[:-5]+video_post_name+'.mp4', fps=20)
    ...
    for rollout_id in range(num_rollouts):
        ...
        # return
        obs = env.reset()
        # print(obs['agentview_image'].shape) # (3, 84, 84)
        # return
        ...
        qpos_history = torch.zeros((1, max_timesteps, input_to_model_low_dim)).cuda()
        ...
        with torch.inference_mode():
            for t in range(max_timesteps):
                ...
                # qpos_numpy = np.array(obs['robot0_joint_pos'])

                if video_writer is not None:
                    if video_count % video_skip == 0:
                        video_img = []
                        for cam_name in mimic_camera_names:
                            video_img.append(env.render(mode="rgb_array", height=512, width=512, camera_name=cam_name))
                        video_img = np.concatenate(video_img, axis=1) # concatenate horizontally
                        video_writer.append_data(video_img)
                    video_count += 1

                eefpos = obs['robot0_eef_pos']
                eefquat = obs['robot0_eef_quat']
                qpos_numpy = np.append(eefpos, eefquat)
                gripper_qpos = obs['robot0_gripper_qpos']
                qpos_numpy = np.append(qpos_numpy, gripper_qpos)
                ...
                obs, reward, _, _ = env.step(target_qpos)
...
def forward_pass(data, policy):
    ...
    image_data, qpos_data, action_data, is_pad = image_data.cuda(), qpos_data.cuda().float(), action_data.cuda().float(), is_pad.cuda()
